...
### Why do I need to include them first?
  # So that the functions are available to call on
...
collect_images <- function(im.path) {
  ...
  file_names <- fs::dir_ls(im.path) # Returns file names 
  ...
  image.files <- data.frame(file_names, ext_names) %>% 
    as_tibble() %>% # Make data into a tibble
    mutate(fn_char = as.character(file_names)) %>%  # Make a column with the file names
    mutate(file_names = fs_path(file_names %>% as.character)) %>% #lost fs type when put into df - making sure file names are consistent
    filter(str_detect(ext_names, "tif")) %>% # filtering all the files that are tifs
    mutate(fn = fs::path_file(file_names)) %>% # Making column of fn that contains the file names
    mutate( # new column
      year = stringr::str_sub(fn, 4, 7), # gets characters 4 through 7 of the file name to find year
      start_month = stringr::str_sub(fn, 9, 10), # gets characters 9 through 10 to find start month
      start_day = stringr::str_sub(fn, 11, 12), # Characters 11 to 12 of fn have start day
      end_month = stringr::str_sub(fn, 14, 15), # Characters 14 to 14 have end month
      end_day = stringr::str_sub(fn, 16, 17) # characters 16 to 17 have end day
    ) %>%
    mutate(
      start_date = paste0(year, "-", start_month, "-", start_day),
      end_date = paste0(year, "-", end_month, "-", end_day) # puts all the dates together in start date and end date to make new columns of full dates
    )
}
...
clip_raster_to_bounding <- function(in.raster.path, boundingPolygon.path) {
  ...
  ras <- terra::rast(in.raster.path) # creating a raster
  
  ras.p4 <- terra::crs(ras) # finding coordinate system of raster
  
  # What's a bounding polygon and how can you know what that poly looks like?
  
  bb <- sf::read_sf(boundingPolygon.path) # Reading a polygon
  
  bb.projected <- sf::st_transform(bb, ras.p4) %>% vect(.) # Making sure crs are same and then making bb as a vect for the terra file
  
  ras.subset <- terra::crop(ras, bb.projected) # Crop raster to the bb shape
}
...
calc_area_by_thresholds <- function(in.raster,
                                    ...
                                    lake_transform) {
  # ??
  bb <- sf::read_sf(boundingPolygon.path) ## read shapefile and make it vector bb
  
  # What's a CRS again? It is the coordinate system
  ras.p4 <- terra::crs(in.raster) # finding crs of raster
  ...
  # raster properties
  raster.res <- terra::res(in.raster) # finding resolution of the raster
  pixel.area.m2 <- raster.res[1] * raster.res[2] # multiplies length and width of each pixel
  
  
  # What does terra::crop do?
  ras.subset <- terra::crop(in.raster, bb.projected) # Cropping the raster to the bounding box again
  ...
  masked_raster <- mask(ras.subset, ras.subset >= 2 &
                          ras.subset <= 249) # Making a mask of the raster that contains only the valid pixels
  ...
  thresh.low <- 20000 / 1e8 # creating vectors designated thresholds for the cyanobacteria value
